...
const STYLE_PROMPTS = Object.freeze({
    brief: "脱离当前角色,使用上帝视角将下面消息楼层进行总结，按时间或逻辑顺序保留关键信息，省略冗余描述",
    balanced: "脱离当前角色,使用上帝视角把以下对话压缩成一条适合长期记忆的总结。保留关键事件、人物关系变化、正在推进的目标、情绪变化、线索和承诺。只输出总结正文。",
    detailed: "脱离当前角色,使用上帝视角详细总结以下对话，并保留可供后续续写使用的长期记忆信息，包括关键事件、人物关系变化、当前目标与障碍、伏笔秘密和承诺、当前场景状态。只输出总结正文。", 
});
